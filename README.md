...
The final model learned using the boosted random forest algorithm yielded a weighted average score of 73% precision, 61% recall and 64% F1-score (harmonic mean of precision and recall). Precision is the proportion of positive samples predicted by the model that are in fact positive samples. Recall is the proportion of true positive samples are correctly predicted by the model. Accuracy is the proportion of predictions, positive and negative, that are correctly classified by the model. 
